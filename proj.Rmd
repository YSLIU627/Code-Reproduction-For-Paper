...
CD <- function(alpha_0 =0,max_iter =1000,sample_p,loss.subgrad,loss.dualres,loss.w_func,A,y,loss,loss.gapwise,loss.gap)
  {
  record.gap <- list(length = max_iter)
  record.opt <- list(length = max_iter)
  n <- length(alpha_0)
  # A,y are given data
  ...
  gap <- 0
  for( j in 1:n){
    gap <- gap + loss.gap(dimension = i,...)
  }
  record.gap[iter] <- gap
  record.opt[iter] <- loss(...)
  return (list(iter,record.gap,record.opt))
  }
}
...
Collect data
```{r}
A = matrix(c(1,1,3,5,5,7),nrow = 3,ncol =2)
alpha = c(4,5)
y <- A%*%alpha
y
CD(alpha_0 = c(0,0),max_iter = 100,A=A,y=y,sample_p = sample.gapwise,loss = lasso.loss ,loss.gap =lasso.gap ,loss.subgrad = lasso.subgrad,loss.duaresl = lasso.dualres,loss.w_func = lasso.w_func )
```
